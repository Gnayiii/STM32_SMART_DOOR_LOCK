...
uint8_t PS_SetAddr(uint32_t PS_addr)
{
    ...
    temp = 0x01 + 0x07 + 0x15 + (PS_addr >> 24) + (PS_addr >> 16) + (PS_addr >> 8) + (uint8_t)PS_addr;
    SendCheck(temp);
...
const char *EnsureMessage(uint8_t ensure)
{
    ...
    case AS608_OK:
        return "OK";
    case AS608_PACKET_RX_ERROR:
        return "数据包接收错误";
    case AS608_NO_FINGER:
        return "传感器上没有手指";
    case AS608_ENROLL_FAIL:
        return "录入指纹图像失败";
    case AS608_IMG_TOO_DRY:
        return "指纹图像太干、太淡而生不成特征";
    case AS608_IMG_TOO_WET:
        return "指纹图像太湿、太糊而生不成特征";
    case AS608_IMG_TOO_MESSY:
        return "指纹图像太乱而生不成特征";
    case AS608_FEW_FEATURE_POINTS:
        return "特征点太少而生不成特征";
    case AS608_NOT_MATCH:
        return "指纹不匹配!";
    case AS608_NOT_FOUND:
        return "指纹不匹配!"; // 模块常混用
    case AS608_MERGE_FAIL:
        return "特征合并失败";
    case AS608_ADDR_OUT_OF_RANGE:
        return "访问指纹库时地址序号超出范围";
    case AS608_DEL_FAIL:
        return "删除模板失败";
    case AS608_EMPTY_FAIL:
        return "清空指纹库失败";
    case AS608_NO_IMAGE:
        return "缓冲区无有效原始图";
    case AS608_FLASH_ERROR:
        return "读写 FLASH 出错";
    case AS608_INVALID_REG_NUM:
        return "无效寄存器号";
    case AS608_INVALID_REG_VAL:
        return "寄存器设定内容错误";
    case AS608_INVALID_NOTE_PAGE:
        return "记事本页码指定错误";
    case AS608_DB_FULL:
        return "指纹库满";
    case AS608_ADDR_ERROR:
        return "地址错误";
    default:
        return "模块返回确认码有误";
    }
