...
void PasswordFlash_Init(void)
{
    ...
    if (*flash_mark != PASSWORD_VALID_MARK_XOR)
    {
        // 初始化默认密码数据
        init_pass.valid_mark = PASSWORD_VALID_MARK_XOR;             // 设置有效标识
        memcpy(init_pass.password, Password_Default, PASSWORD_LEN); // 默认密码
        for (uint8_t i = 0; i < PASSWORD_LEN; i++)
            init_pass.password[i] ^= PASSWORD_XOR_KEY; // 异或加扰
...
uint8_t PasswordFlash_Write(const char *new_pass)
{
    ...
    new_data.valid_mark = PASSWORD_VALID_MARK_XOR; // 设置有效标识
    for (uint8_t i = 0; i < PASSWORD_LEN; i++)
        new_data.password[i] = new_pass[i] ^ PASSWORD_XOR_KEY; // 逐字节异或加扰
...
uint8_t PasswordFlash_Read(char *buff)
{
    ...
    // 校验有效标识（确保数据未损坏）
    if (stored_pass->valid_mark != PASSWORD_VALID_MARK_XOR)
    {
        return 0; // 数据无效，返回失败
    }
    ...
    for (uint8_t i = 0; i < PASSWORD_LEN; i++)
        buff[i] ^= PASSWORD_XOR_KEY; // RAM内还原明文，两次异或同一个数会还原为原本值
